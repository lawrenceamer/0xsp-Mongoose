...
function executesolojob(cmd:string):string;
...
Begin
  ...
  for i := 1 to paramcount do begin
      ...
      if(paramstr(i)='-o') then begin
          writeln('[+] Found Results has been Sent To :> ',paramstr(i+1));   //here we can add host as CONST
          host :=paramstr(i+1);
          rep := true;
      end;
...
try
   url := api+'/0xsp/upload/uploadFile';        //API IP , Default Path of End-Point
